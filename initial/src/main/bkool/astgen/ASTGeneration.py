...
class ASTGeneration(BKOOLVisitor):
    ...
    def visitMutDecl(self, ctx: BKOOLParser.MutDeclContext):
        ...
        result += str(
            AttributeDecl(
                kind,
                VarDecl(
                    Id(ctx.ID(0).getText()),  # type:ignore
            ...
            for i in range(1, size + 1):
                result += "," + str(
                    AttributeDecl(
                        kind,
                        VarDecl(
                            Id(ctx.ID(i).getText()),  # type:ignore
                            ctx.typ().accept(self),
    ...
    def visitPara(self, ctx: BKOOLParser.ParaContext):
        # para: typ ID (CM ID)*;
        result = []
        result += [VarDecl(Id(ctx.ID(0).getText()), ctx.typ().accept(self))]

        if ctx.CM():
            size = len(ctx.CM())
            for i in range(1, size + 1):
                result += [VarDecl(Id(ctx.ID(i).getText()), ctx.typ().accept(self))]
        return result
    ...
    def visitExp(self, ctx: BKOOLParser.ExpContext):
        ...
        if ctx.getChildCount() == 3:
            if ctx.LESS():
                return BinaryOp(
                    "<", ctx.equalee(0).accept(self), ctx.equalee(1).accept(self)
                )
            if ctx.LEQ():
                return BinaryOp(
                    "<=", ctx.equalee(0).accept(self), ctx.equalee(1).accept(self)
    ...
    def visitConCatee(self, ctx: BKOOLParser.ConCateeContext):
        ...
        if ctx.getChildCount() == 3:
            return BinaryOp("^", ctx.conCatee().accept(self), ctx.notee().accept(self))
        else:
    ...
    def visitSubAddee(self, ctx: BKOOLParser.SubAddeeContext):
        ...
        if ctx.getChildCount() == 2:
            return UnaryOp("+" if ctx.ADD() else "-", ctx.subAddee().accept(self))
        else:
    ...
    def visitIndexee(self, ctx: BKOOLParser.IndexeeContext):
        ...
        if ctx.getChildCount() == 4:
            return ArrayCell(ctx.memAccessee().accept(self), ctx.exp().accept(self))
        else:
    ...
    def visitNewee(self, ctx: BKOOLParser.NeweeContext):
        ...
        return NewExpr(
            Id(ctx.ID().getText()), ctx.argLits().accept(self) if ctx.argLits() else []
        )
    ...
    def visitStmt(self, ctx: BKOOLParser.StmtContext):
        # stmt: blockStmt | asmStmt | ifStmt | forStmt | breakStmt | continueStmt | returnStmt | invokeStmt;
        if ctx.blockStmt():
    ...
    def visitBlockStmt(self, ctx: BKOOLParser.BlockStmtContext):
        ...
        return Block(
            [decl for x in ctx.nullAbleDeclList() for decl in self.visit(x)]
            if ctx.nullAbleDeclList()
            else [],
        ...
        if ctx.FINAL():
            result.append(
                ConstDecl(
                    Id(ctx.ID(0).getText()),
                    ctx.typ().accept(self),
                    ctx.atrbInit(0).accept(self) if ctx.atrbInit() else None,
                )
            )
            if ctx.CM():
                size = len(ctx.CM())
                for i in range(1, size + 1):
                    result.append(
                        ConstDecl(
                            Id(ctx.ID(i).getText()),
                            ctx.typ().accept(self),
                            ctx.atrbInit(i).accept(self) if ctx.atrbInit() else None,
                        )
                    )
        else:
            result.append(
